...
async function bhActivities(){if(window.__bhActivities)return window.__bhActivities;const r=await fetch("data/activities.json",{cache:"no-store"});if(!r.ok)throw new Error("Could not load activities");window.__bhActivities=await r.json();return window.__bhActivities}
